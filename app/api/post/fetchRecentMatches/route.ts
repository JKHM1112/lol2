...
export async function POST(request: NextRequest) {
    ...
    const { queue, start, games } = data;

    try {

        const db = (await connectDB).db('dream');
        const saveGameNicknameCollection = db.collection('saveGameNickname');
        const puuidList = await saveGameNicknameCollection.find({}).toArray();

        const levelUpEventsByParticipant: { [key: string]: any[] } = {};
        for (const puuidObj of puuidList) {
            const puuid = puuidObj.puuid; // 각 puuid 사용
